...
bool right_detected(){
	clearTimer(T3);
	while(time1(T3)< 1000){ // if left doesn't detect within 1 seconds since right is detected, break and return false
		move_right(100);//move right until right is detected
		int _sensorDetect = sensorDetect();
		if (_sensorDetect == 1110 || _sensorDetect == 1011 || _sensorDetect ==  1111){ //returns to the circular search if top is picking up something
			return false;
		}
		if (_sensorDetect = 1100) {
			bool temp=left_detected();
			return temp;
	}
	return false;
}
