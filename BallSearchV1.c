...
#pragma config(Motor,  port10,          collectionMotor, tmotorVex393_HBridge, openLoop)
...
int sensorDetect()
{
	...
	if ((avgRglobal < 50) && (avgRglobal > 10))
	{
		returnVal=returnVal+1;
	}
	if((avgLglobal < 50) && (avgLglobal > 10)){
		returnVal=returnVal+100;
	}
	if((avgTLglobal < 50) && (avgTLglobal > 10)){
...
bool clockwise_circular_search_right(int milliSecond)
{
	...
	while(time1(T2)< milliSecond){
		...
		if (_sensorDetect==1000)
		{
			...
			move_forward_with_collection(500);
			hogCPU();
			orientSouth();
			releaseCPU();
			return true;
